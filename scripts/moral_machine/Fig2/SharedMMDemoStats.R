...
plotdata.gender <- demo.gender %>%
  group_by(Review_gender) %>%
  summarise(count = round(length(UserID) / nrow(demo.gender), 2))
...
plotdata.income <- demo.income %>%
  group_by(Review_income) %>%
  summarise(count = round(length(UserID) / nrow(demo.income), 2))
...
plotdata.education <- demo.education %>%
  group_by(Review_education) %>%
  summarise(count = round(length(UserID) / nrow(demo.education), 2))
...
demo.age <- demo[-which(demo$Review_age < 1 |
                          demo$Review_age > 100 |
                          is.na(demo$Review_age)),]
