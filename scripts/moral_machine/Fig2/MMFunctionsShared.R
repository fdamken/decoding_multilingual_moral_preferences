...
GetMainEffectSizes.Characters <- function(profiles, charType, k) {
  ...
  for (i in 1:k) {
    Temp <- profiles[which(profiles$ScenarioType == charType &
                             profiles$ScenarioTypeStrict == charType &
                             profiles$NumberOfCharacters == i),]
...
PlotAndSave.Characters <- function(plotdata, charType, isMainFig, filename) {
  if (isMainFig) {
    ...
    gg <- ggplot(plotdata, aes(CharacterType, Estimates, color = Estimates > 0, fill = Estimates > 0)) +
      ...
      theme_bw() +
      coord_flip() +
      labs(title = "Preference in favor of sparing characters") +
      xlab("") +
      ylab(expression(paste("\n ", Delta, " Pr"))) +
    ...
    gg <- ggplot(plotdata, aes(CharacterType, Estimates, color = `No. of Characters`, fill = `No. of Characters`)) +
      ...
      theme_bw() +
      coord_flip() +
      labs(title = paste0("Effect of characters within ", charType, " dimension")) +
      xlab("") +
      ylab(expression(paste("\n ", Delta, " Pr"))) +
      ylim(-0.7, 0.7) +
...
GetEffectSizes.Inter.Att <- function(profiles, r, Attr) {
  ...
  for (j in 0:(k - 1)) {
    if (Attr != "Intervention") {
      # For intervention
      if (Attr == "Barrier")
        profile.Int <- profiles[which(profiles$Barrier == j &
                                        profiles$CrossingSignal == 0 &
                                        profiles$PedPed == 0),]
      if (Attr == "CrossingSignal")
        profile.Int <- profiles[which(profiles$CrossingSignal == j &
                                        profiles$Barrier == 0 &
                                        profiles$PedPed == 0),]
      ...
      # For relationship to vehicle
      profile.Relation <- profiles[which(profiles$CrossingSignal == 0 &
                                           profiles$PedPed == 0 &
      ...
      # Legality
      profile.Legality <- profiles[which(profiles$CrossingSignal != 0 &
                                           profiles$PedPed == 1 &
                                           profiles$Intervention == j),]
      profile.Legality$CrossingSignal <- factor(profile.Legality$CrossingSignal, levels = levels(profiles$CrossingSignal)[2:3])
      profile.Legality$BC.weights <- calcWeightsTheoretical(profile.Legality)
    ...
    for (i in 1:6) {
      if (Attr == "Intervention")
        Temp <- profiles[which(profiles$ScenarioType == lev[i] &
                                 profiles$ScenarioTypeStrict == lev[i] &
                                 profiles$Intervention == j),]
      if (Attr == "Barrier")
        Temp <- profiles[which(profiles$ScenarioType == lev[i] &
                                 profiles$ScenarioTypeStrict == lev[i] &
...
GetFilteredList <- function(profiles, filter, N) {
  a <- profiles %>%
    group_by_(filter) %>%
    summarise(count = length(unique(UserID)))
