...
b <- profiles.S %>%
  group_by(UserID, ExtendedSessionID) %>%
  summarise(count = length(PreferredIsChosen))
...
## Intervention
Temp <- profiles.S.C
lm.Temp.I <- felm(StayIsChosen ~ as.factor(CrossingSignal) +
  as.factor(Barrier) +
  Review_gender +
  Review_age.z +
  Review_ContinuousIncome.z +
  Review_CollegeEducated +
  Review_political.z +
  Review_religious.z |
  0 |
  0 |
...
for (i in 1:6) {
  ...
  lm.Temp[[i]] <- felm(PreferredIsChosen ~ as.factor(Intervention) +
    as.factor(CrossingSignal) +
    as.factor(Barrier) +
    Review_gender +
    Review_age.z +
    Review_ContinuousIncome.z +
